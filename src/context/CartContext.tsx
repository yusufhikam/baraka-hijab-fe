...
type CartContextType = {
    ...
    subTotal: number;
    subTotalPerVariant: (id: number) => void;
...
export const CartProvider = ({ children }: CartProviderProps) => {
    ...
    const { data: carts = [], isLoading, isError, refetch: refreshCartData } = useQuery({
    ...
    const groupedCarts: groupedCartType[] | undefined = useMemo(() => {
        return carts?.reduce((acc: groupedCartType[], cart) => {
            ...
            const existingGroup = acc.find((group) => group.product.id === productId);
            if (existingGroup) {
    ...
    const subTotal = useMemo(() => {
        return carts.reduce((acc, cart) => acc + (cart.productVariant.product.price * cart.quantity), 0);
    }, [carts])

    // menghitung subTotal per product variant, // when called wrap with String()
    const subTotalPerVariant = useCallback((id: number) => {
        const total = carts.filter((cart) => cart.productVariant.id === id).reduce((acc, cart) => acc + (cart.productVariant.product.price * cart.quantity), 0);
    ...
    const formatCurrency = (number: number) => {
        ...
        return new Intl.NumberFormat('en-US', { style: "currency", currency: "IDR", minimumFractionDigits: 0 }).format(number);
    }

    return (
        <CartContext.Provider value={{ carts, cartsLength, subTotal, refreshCartData, isLoading, isError, groupedCarts, subTotalPerVariant, formatCurrency }}>
            {children}
