...
const CartItems = memo(() => {
	...
	return (
		<div
			className={`cart-items-container gap-3 overflow-x-auto rounded-md p-5 shadow-lg shadow-black/25 ${isDarkMode ? "bg-slate-700" : "bg-white"} ${isLoading && "animate-pulse cursor-wait"}`}
		>
			<h1 className="font-poppins-semibold mb-5 text-2xl">Cart Items</h1>

			{isLoading ? (
				<Loader size={40} className="mx-auto my-5 animate-spin text-red-500" />
			) : (
				<table className="w-full table-auto">
					<thead className="border-b-2 text-lg">
						<tr className="">
							<th className="text-center sm:text-left">Product</th>
								...
								>
											...
											<div className="product-details flex w-full flex-col justify-between">
												<div className="flex flex-col">
													<h1 className="text-xs font-bold opacity-35">
														{
															cart.productVariant.product.subCategory.category
																.name
														}
													</h1>
												...
												<div className="flex flex-col flex-wrap gap-2 sm:flex-row sm:items-center">
													<div className="mt-5 flex items-center gap-2">
														<div
															...
															style={{
																backgroundColor: cart.productVariant.color,
															}}
														></div>
														<span>|</span>
														<p className="">{cart.productVariant.size}</p>
													</div>
													<div className="flex items-center gap-1 sm:hidden">
														...
														<h1 className="font-semibold">
															{String(
																formatCurrency(
																	cart.productVariant.product.price,
																),
															)}
														</h1>
													</div>
									...
									<td className="price hidden px-1 sm:table-cell">
										<h1 className="my-auto text-center">
											...
											}).format(cart.productVariant.product.price)}
										</h1>
									</td>
									...
									<td className="price hidden px-1 sm:table-cell">
										<h1 className="text-center font-bold">
											{String(subTotalPerVariant(cart.productVariant.id))}
										</h1>
									</td>
