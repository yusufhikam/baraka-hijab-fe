...
export const handleAddToCart = async (
    ...
    const totalQty = Object.values(quantity).reduce(
        (acc, qty) => acc + (qty || 0),
        0
    )
    ...
    if (isAuthenticated) {
        try {
            for (const variant of selectedProductQuickView?.product_variants ??
                []) {
        ...
        // jika belum login data disimpan di localstorage 'carts'
        ...
        selectedProductQuickView?.product_variants.forEach((variant) => {
            const qty = quantity[variant.id]
                    ...
                    newCart.push({
                        ...
                        productVariant: {
                            ...
                            product: {
                                ...
                                subCategory: {
                                    id: selectedProductQuickView.sub_category
                                        .id,
                                    name: selectedProductQuickView.sub_category
                                        .name,
                                    category: {
                                        id: selectedProductQuickView
                                            .sub_category.category.id,
                                        name: selectedProductQuickView
    ...
    const resetQuantity = Object.fromEntries(
        (selectedProductQuickView?.product_variants ?? []).map((variant) => [
            variant.id,
            0,
        ])
    )
